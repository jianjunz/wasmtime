...
impl LMSDiscreteScheduler {
    pub fn new(inference_steps: usize, config: LMSDiscreteSchedulerConfig) -> Self {
        ...
        let alphas: Vec<f64> = betas.iter().map(|&x| 1. - x).collect();
        let alphas_cumprod: Vec<f64> = alphas
            .iter()
            .scan(1.0, |state, &x| {
                *state *= x;
                Some(*state)
        ...
        let sigmas = Self::interp(
            ...
            ndarray::range(0., sigmas.len() as f64, 1.)
                .collect::<Vec<f64>>()
                .as_slice(),
            sigmas.as_slice(),
        );
        ...
        let init_noise_sigma: f64 = sigmas.iter().fold(f64::MIN, |a, &b| a.max(b));
    ...
    pub fn scale_model_input(&self, sample: Vec<f64>, timestep: f64) -> Vec<f32> {
    ...
    fn interp(timesteps: &[f64], range: &[f64], sigmas: &[f64]) -> Vec<f64> {
        let sz = timesteps.len();
        let mut result: Vec<f64> = vec![0.; sz];
    ...
    pub fn step(&mut self, model_output: &[f64], timestep: f64, sample: &[f64]) -> Vec<f64> {
        let step_index = self.timesteps.iter().position(|&t| t == timestep).unwrap();
        let sigma = self.sigmas[step_index];

        // 1. compute predicted original sample (x_0) from sigma-scaled predicted noise
        let pred_original_sample: Vec<f64> = match self.config.prediction_type {
        ...
        let mut derivative: Vec<f64> = vec![0.0; sample.len()];
        for i in 0..sample.len() {
        ...
        let deriv_prod: Vec<Vec<f64>> = lms_coeffs
            .iter()
            .zip(self.derivatives.iter().rev())
            .map(|(coeff, derivative)| {
                let mut result: Vec<f64> = vec![0.; sample.len()];
            ...
            .collect();
        let mut deriv_sum: Vec<f64> = vec![0.; sample.len()];
    ...
    pub fn add_noise(&self, original_samples: &[f64], noise: &[f64], timestep: f64) -> Vec<f64> {
        let step_index = self.timesteps.iter().position(|&t| t == timestep).unwrap();
        let sigma = self.sigmas[step_index];

        // noisy samples
        let mut samples: Vec<f64> = vec![0.; original_samples.len()];
